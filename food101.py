...
image_processor = ViTImageProcessorFast(size=224, do_normalize=False)

# 应用数据预处理
def transform_images(examples):
    examples['image'] = [image_processor(img, return_tensors="pt")['pixel_values'].squeeze() for img in examples['image']]
    return examples

dataset = load_dataset("food101")
dataset = dataset.map(transform_images, batched=True, num_proc=32)
