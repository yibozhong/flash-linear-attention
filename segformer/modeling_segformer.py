...
class SegformerLayer(nn.Module):
    ...
    def __init__(self, config, hidden_size, num_attention_heads, drop_path, sequence_reduction_ratio, mlp_ratio, gla=False):
        super().__init__()
        ...
        if not gla:
            ...
            # self.attention = GatedSlotAttention(
            #     mode=config.attn_mode,
            #     hidden_size=hidden_size,
            #     expand_k=config.expand_k,
            #     expand_v=config.expand_v,
...
class SegGLAModel(SegformerPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        # hierarchical Transformer encoder
        self.encoder = SegGLAEncoder(config)
...
class SegGLAForSemanticSegmentation(SegformerPreTrainedModel):
    def __init__(self, config, **kwargs):
        super().__init__(config)
        self.segformer = SegGLAModel(config)
        self.decode_head = SegformerDecodeHead(config)
        # Initialize weights and apply final processing
        self.post_init()
